    ...
    public bool IsValidSudoku(char[][] board) 
    {
        ...
        for (var i = 0; i < 9; i++)
        {
            ...
            var columnCount =
                ...
                (board[7][i] == '.' ? 0 : 1) +
                (board[8][i] == '.' ? 0 : 1);
            Console.WriteLine(board[i][8] == '.' ? 0 : 1);
            Console.WriteLine($"i: {i}, row.Count:{row.Count}");
            Console.WriteLine($"i: {i}, rowCount: {rowCount}");
            Console.WriteLine($"i: {i}, column.Count: {column.Count}");
            Console.WriteLine($"i: {i}, columnCount: {columnCount}");
            ...
            if ((i + 1) % 3 == 0)
            {
                Console.WriteLine($"first square count: {firstSquare.Count}");
                Console.WriteLine($"first count: {firstCount}");
                Console.WriteLine($"second square count: {secondSquare.Count}");
                Console.WriteLine($"second square count: {secondCount}");
                Console.WriteLine($"third square count: {thirdSquare.Count}");
                Console.WriteLine($"third count: {thirdCount}");
